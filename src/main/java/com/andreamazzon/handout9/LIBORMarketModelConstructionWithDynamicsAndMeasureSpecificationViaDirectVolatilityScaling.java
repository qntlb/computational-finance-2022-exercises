 ...
 * terminal or the spot one. However, this specific class has a bug: we will have
 * a look at it together and try to fix it.
 *
...
public class LIBORMarketModelConstructionWithDynamicsAndMeasureSpecificationViaDirectVolatilityScaling {
	...
	public static final TermStructureMonteCarloSimulationModel createLIBORMarketModel(int numberOfPaths,
		 ...
		 * 7) combine steps 2, 3, 6 to create the LIBOR model, also adding the appropriate
		 * properties about dynamics and measure
		 * 8) create a Euler discretization of the model we defined in step 7, specifying the model
		 * itself and a Brownian motion that uses the time discretization defined in step 1
		 * 9) give the Euler scheme to the constructor of LIBORMonteCarloSimulationFromLIBORModel, to create an
