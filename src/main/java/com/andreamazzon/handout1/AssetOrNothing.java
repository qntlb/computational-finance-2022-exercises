 ...
 * or nothing" option, that is, an option on an underlying S such that the
 * payoff at maturity T is S(T) 1_{S(T)>K}. Note that we have to implement
 * only the method getValue(final double evaluationTime, final
 * AssetModelMonteCarloSimulationModel model), besides the constructor.
 *
...
public class AssetOrNothing extends AbstractAssetMonteCarloProduct {
	...
	// it is useful in case of a multi-dimensional process. Otherwise, it is 0
