 ...
 * the terminal or the spot one. We can also specify the number of factors we want to consider: we can perform
 * factor reduction.
 *
 * @author Andrea Mazzon
 *
 */
public class LIBORMarketModelConstructionWithFactorReduction {
	 ...
	 * @param numberOfFactors, the number of factors considered in the model: if this is equal to
	 * LIBORPeriodDiscretization.getNumberOfTimes()-1, then there is no factor reduction.
	...
	createLIBORMarketModel(int numberOfPaths,
		 ...
		 */
		 final double parameterForBlended = (dynamics == Dynamics.LOGNORMAL) ? 0.0 : 1.0;
		...
		final String nameOfTheStateSpace = "normal";
		 ...
		 */
		properties.put("stateSpace", nameOfTheStateSpace);
	...
	public static TermStructureMonteCarloSimulationModel getCloneWithModifiedCorrelation(
		...
		final IndependentIncrements oldBrownianMotion = oldLIBORSimulation.getBrownianMotion();
