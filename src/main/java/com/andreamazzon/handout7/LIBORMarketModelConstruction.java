...
public class LIBORMarketModelConstruction {
	...
	public static TermStructureMonteCarloSimulationModel createLIBORMarketModel(int numberOfPaths,
		 ...
		 * following steps: 1) provide the time discretization for the evolution of the
		 * processes 2) provide the time discretization of the tenure structure 3)
		 * provide the observed term structure of the initial LIBOR rates (also called
		 ...
		 * the LIBOR processes 4) create the volatility structure, i.e., the terms
		 * sigma_i(t_j) in dL_i(t_j)=\sigma_i(t_j)L_i(t_j)dW_i(t_j) 5) create the
		 * correlation structure, i.e., define the terms \rho_{i,j}(t) such that
		 * d<W_i,W_k>(t)= \rho_{i,k}(t)dt 6) combine all steps 1, 2, 4, 5 to create a
		 * covariance model 7) combine steps 2, 3, 6 to create the LIBOR model 8) create
		 * a Euler discretization of the model we defined in step 7, specifying the
		 * model itself and a Brownian motion that uses the time discretization defined
		 * in step 1 9) give the Euler scheme to the constructor of
		 * LIBORMonteCarloSimulationFromLIBORModel, to create an object of type
